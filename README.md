...
#### Role
Pulls a given `.warc.gz` file from the url given by `?archive-url` and serves it with the headers needed to playback including:
...
#### Source
[archive-warc-gz.js](/netlify/edge-functions/archive-warc-gz.js)

#### Query parameters
| Name | Required ? | Description |
| --- | --- | --- |
| `archive-url` | Yes | Full url to the `.warc.gz` file to embed. Must point to a host listed in [allowlist](#allowlist). |

### /archive.wacz

#### Role
Pulls a given `.wacz` file from the url given by `?archive-url` and serves it with the headers needed to playback including:
- `access-control-allow-origin`
- `accept-ranges`
- `content-type`
- `content-disposition`
...
`.wacz` files need to be hosted on a server supporting [range requests](https://developer.mozilla.org/en-US/docs/Web/HTTP/Range_requests): `archive-wacz.js` will try to detect support for range requests, and provide a basic polyfill for it if not.

#### Methods
`GET`, `HEAD`

#### Source
[archive-wacz.js](/netlify/edge-functions/archive-wacz.js)

#### Query parameters
| Name | Required ? | Description |
| --- | --- | --- |
| `archive-url` | Yes | Full url to the `.wacz` file to embed. Must point to a host listed in [allowlist](#allowlist). |
